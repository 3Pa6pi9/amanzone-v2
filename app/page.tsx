...
import { 
  ...
  LayoutGrid, List as ListIcon, Bot, Send
} from "lucide-react";
...
const initialSettingsState = {
  ...
  logoUrl: "", phone: "", email: "", address: "Addis Ababa, Ethiopia",
  taxRate: 15, deliveryBaseFee: 250, aiEnabled: false
};
...
export default function PremiumStorefront() {
  ...
  const [chatMessages, setChatMessages] = useState<{role: 'user' | 'ai', text: string}[]>([
    { role: 'ai', text: "Hello! I am the AmanZone AI Assistant. I can check our live inventory or help you find specific materials. What do you need today?" }
  ]);
  ...
  useEffect(() => {
    let ticking = false;
    const handleScroll = () => { if (!ticking) { window.requestAnimationFrame(() => { setScrolled(window.scrollY > 50); ticking = false; }); ticking = true; } };
  ...
  const catalogTree = useMemo(() => {
    ...
    products.forEach(p => { const m = p.menu || "Uncategorized"; const sm = p.submenu || "General"; const tType = p.type || "Standard"; if (!tree[m]) tree[m] = {}; if (!tree[m][sm]) tree[m][sm] = new Set(); tree[m][sm].add(tType); });
    return tree;
  }, [products]);
  ...
  const addToCart = (product: any) => {
    setCartItems(prev => { const existing = prev.find(item => item.id === product.id); if (existing) return prev.map(item => item.id === product.id ? { ...item, quantity: item.quantity + 1 } : item); return [...prev, { ...product, quantity: 1 }]; });
  ...
  const updateCartQuantity = (id: string, newQuantity: number) => { if (newQuantity < 1) return; setCartItems(prev => prev.map(item => item.id === id ? { ...item, quantity: newQuantity } : item)); };
  ...
  const handleCheckout = async (e: React.FormEvent) => {
    ...
    try {
      const payload = { items: cartItems, customerName: formData.name, companyName: formData.companyName, tinNumber: formData.tinNumber, requireVat: formData.requireVat, phone: formData.phone, totalAmount: cartSubtotal, deliveryType, region: formData.region, subCity: formData.subCity, specificAddress: formData.address, baseDeliveryFee: systemSettings.deliveryBaseFee };
  ...
  const handleChatSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Silently alert the admin if the AI flagged this conversation
      if (data.adminAlert) {
        await addDoc(collection(db, "ai_comms"), {
          alert: data.adminAlert,
          userMessage: userMsg,
      ...
      setChatMessages(prev => [...prev, { role: 'ai', text: "Sorry, my neural link to the database is experiencing interference. Please call our sales team directly." }]);
    } finally {
      setIsAiTyping(false);
    }
  };
  ...
  const renderSlogan = () => {
    ...
    return (
      <>
        {parts[0]}. <br />
        <span className="bg-clip-text text-transparent" style={{ backgroundImage: 'linear-gradient(to right, var(--text-main), var(--accent))' }}>{parts.slice(1).join('.').trim()}</span>
      </>
    );
  };
    ...
    <div className="relative min-h-screen font-sans scroll-smooth" style={{ backgroundColor: 'var(--bg-primary)', color: 'var(--text-main)' }}>
      ...
      <header className={`fixed top-0 w-full z-40 transition-colors duration-300 border-b transform-gpu ${scrolled ? 'bg-black/80 py-3 border-white/10 shadow-2xl backdrop-blur-md' : 'bg-transparent py-3 md:py-5 border-transparent'}`}>
        <div className="max-w-[1400px] mx-auto px-4 md:px-6 flex justify-between items-center">
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => window.scrollTo(0,0)}>
            ...
            <h1 className="text-xl md:text-2xl font-black tracking-tighter truncate max-w-[150px] md:max-w-none">{systemSettings.companyName.split(' ')[0]}<span style={{ color: 'var(--accent)' }}>.</span></h1>
          </div>
      ...
      <main className="relative z-10 max-w-[1400px] mx-auto px-4 md:px-6 pt-28 md:pt-40 pb-10 md:pb-16 flex flex-col items-center text-center">
        ...
        <div className="relative w-full max-w-xl mt-4 md:mt-8">
          ...
          <input type="text" placeholder={t("Search materials...", "ፈልግ...")} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-12 pr-6 py-3 md:py-4 rounded-2xl bg-black/40 border border-white/10 outline-none focus:border-white/30 transition-colors shadow-2xl text-base md:text-lg" />
        </div>
      </main>
                ...
                <div key={item.id} className={`group rounded-[1.5rem] md:rounded-[2rem] p-4 md:p-6 bg-[#111111] border border-white/10 transition-transform duration-300 hover:shadow-2xl relative overflow-hidden animate-in fade-in zoom-in transform-gpu ${viewMode === 'grid' ? 'flex flex-col hover:-translate-y-2' : 'flex flex-row gap-4 md:gap-6 items-center hover:-translate-y-1'}`} style={{ animationDelay: `${(i % 10) * 30}ms` }}>
                  {item.imageUrl ? (
                    <div className={`overflow-hidden bg-black/40 border border-white/5 relative flex-shrink-0 ${viewMode === 'grid' ? 'w-full h-48 md:h-56 mb-4 md:mb-6 rounded-[1rem] md:rounded-2xl' : 'w-24 h-24 md:w-32 md:h-32 rounded-xl md:rounded-2xl'}`}>
                      ...
                      {viewMode === 'grid' && <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4"><p className="text-[10px] md:text-xs font-bold text-white line-clamp-3">{item.description || "Premium structural material ready for deployment."}</p></div>}
                    </div>
                  ) : (
                    <div className={`overflow-hidden bg-black/40 border border-white/5 flex flex-col items-center justify-center text-white/30 flex-shrink-0 ${viewMode === 'grid' ? 'w-full h-48 md:h-56 mb-4 md:mb-6 rounded-[1rem] md:rounded-2xl' : 'w-24 h-24 md:w-32 md:h-32 rounded-xl md:rounded-2xl'}`}><ImageIcon size={24} className={viewMode === 'grid' ? "mb-2 md:w-8 md:h-8" : "md:w-6 md:h-6"} />{viewMode === 'grid' && <span className="text-[10px] md:text-xs uppercase tracking-widest font-bold">No Showcase Asset</span>}</div>
                  )}
                  ...
                  <div className={`flex flex-col flex-1 ${viewMode === 'list' ? 'justify-center min-w-0 py-1' : ''}`}>
                    ...
                    <h4 className={`font-bold group-hover:text-white transition-colors truncate ${viewMode === 'grid' ? 'text-lg md:text-xl mb-1' : 'text-base md:text-lg mb-0.5'}`}>{item.title || "Unnamed Material"}</h4>
                    ...
                    <div className={`mt-auto flex justify-between items-center ${viewMode === 'grid' ? 'pt-4 border-t border-white/10' : ''}`}>
                      <div className="flex flex-col truncate pr-2"><span className={`font-black text-emerald-400 leading-none truncate ${viewMode === 'grid' ? 'text-xl md:text-2xl' : 'text-lg md:text-xl'}`}>{(parseFloat(item.price) || 0).toLocaleString()}</span><span className="text-[8px] md:text-[10px] opacity-50 uppercase tracking-widest mt-1 truncate">ETB / {item.metric || "Unit"}</span></div>
                      <button onClick={() => addToCart(item)} className={`rounded-[1rem] bg-white/5 hover:text-white transition-transform active:scale-95 border border-white/10 shadow-lg hover:bg-[var(--accent)] flex-shrink-0 ${viewMode === 'grid' ? 'p-2.5 md:p-3.5' : 'p-2 md:p-3'}`}><ShoppingCart size={18} className="md:w-5 md:h-5" /></button>
                    </div>
                  </div>
                </div>
              ))}
      ...
      {/* --- AI CHAT WIDGET (ONLY VISIBLE IF ENABLED IN SETTINGS) --- */}
      {systemSettings.aiEnabled && (
        <>
          {/* Chat Window */}
          {isChatOpen && (
            <div className="fixed bottom-24 right-4 md:bottom-28 md:right-6 w-[calc(100vw-32px)] md:w-[380px] h-[500px] max-h-[70vh] bg-[#0A0A0F] border border-white/10 rounded-[1.5rem] shadow-2xl flex flex-col overflow-hidden z-50 animate-in slide-in-from-bottom-10">
              {/* Chat Header */}
              <div className="bg-indigo-900/30 border-b border-indigo-500/20 p-4 flex justify-between items-center">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-indigo-500 flex items-center justify-center text-white"><Bot size={16}/></div>
                  <div>
                    <h3 className="font-bold text-sm text-indigo-100">AmanZone AI</h3>
                    <p className="text-[9px] uppercase tracking-widest text-indigo-400">Live Assistant</p>
                  </div>
                </div>
                <button onClick={() => setIsChatOpen(false)} className="text-white/50 hover:text-white"><X size={18}/></button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4" ref={chatScrollRef}>
                {chatMessages.map((msg, idx) => (
                  <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[85%] p-3 rounded-2xl text-sm ${msg.role === 'user' ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-white/10 text-gray-200 rounded-bl-none'}`}>
                      {msg.text}
                    </div>
                  </div>
                ))}
                {isAiTyping && (
                  <div className="flex justify-start">
                    <div className="max-w-[85%] p-3 rounded-2xl bg-white/5 rounded-bl-none text-white/50 text-sm flex items-center gap-2">
              ...
              <form onSubmit={handleChatSubmit} className="p-3 border-t border-white/10 bg-black flex gap-2">
                <input 
                  type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} placeholder="Ask about materials or stock..."
                  className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-sm outline-none focus:border-indigo-500 transition-colors"
                />
                <button type="submit" disabled={isAiTyping || !chatInput.trim()} className="bg-indigo-600 disabled:opacity-50 hover:bg-indigo-500 text-white p-2 w-10 h-10 flex items-center justify-center rounded-xl transition-colors">
                  <Send size={16}/>
                </button>
              </form>
            </div>
          ...
          <button 
            ...
            className={`fixed bottom-4 left-4 md:bottom-6 md:left-6 z-40 w-12 h-12 md:w-14 md:h-14 rounded-full shadow-2xl flex items-center justify-center transition-transform hover:scale-110 ${isChatOpen ? 'bg-indigo-800' : 'bg-indigo-600'}`}
          >
      ...
      <div className="fixed bottom-4 right-4 md:bottom-6 md:right-6 z-30 flex flex-col gap-4">
        <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className={`p-2.5 md:p-3 rounded-full bg-black/80 backdrop-blur-md border border-white/20 shadow-xl transition-all duration-300 ${scrolled ? 'translate-x-0 opacity-100' : 'translate-x-20 opacity-0'}`}><ChevronDown size={18} className="rotate-180 md:w-5 md:h-5" /></button>
      </div>
        ...
        <div className={`absolute top-0 right-0 h-full w-full md:w-[500px] bg-[#050505] border-l border-white/10 shadow-2xl transform transition-transform duration-300 ease-out flex flex-col ${isCartOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          {checkoutStep === 1 ? (
                    ...
                    <div className="flex-1 space-y-3 md:space-y-4">
                      {cartItems.map(item => (
                        <div key={item.id} className="flex gap-3 md:gap-4 items-center bg-[#111111] border border-white/10 p-2.5 md:p-3 rounded-2xl">
                          <div className="w-14 h-14 md:w-16 md:h-16 rounded-xl bg-black/50 border border-white/5 overflow-hidden flex-shrink-0"><img src={item.imageUrl} alt={item.title} loading="lazy" className="w-full h-full object-cover" /></div>
                          <div className="flex-1 min-w-0"><h4 className="font-bold text-xs md:text-sm mb-0.5 truncate">{item.title}</h4><p className="text-[10px] md:text-xs opacity-70 mb-1 md:mb-2">{(parseFloat(item.price) || 0).toLocaleString()} ETB</p><div className="flex items-center gap-2 mt-1"><span className="text-[9px] md:text-[10px] font-bold opacity-50 uppercase tracking-widest">QTY:</span><input type="number" min="1" value={item.quantity || 1} onChange={(e) => updateCartQuantity(item.id, parseInt(e.target.value) || 1)} className="w-14 md:w-16 px-1 md:px-2 py-0.5 bg-black border border-white/10 rounded text-[10px] md:text-xs font-bold text-center outline-none focus:border-[var(--accent)]" /></div></div>
                          <div className="flex flex-col items-end justify-between h-full py-1"><button onClick={() => removeFromCart(item.id)} className="text-red-400/50 hover:text-red-400 transition-colors mb-2 p-1"><Trash2 size={14} /></button><span className="font-black text-emerald-400 text-xs md:text-sm">{((parseFloat(item.price) || 0) * item.quantity).toLocaleString()}</span></div>
                        </div>
                      ))}
                    </div>
                  )}
            ...
            <form id="checkout-form" onSubmit={handleCheckout} className="flex-1 flex flex-col overflow-hidden">
                ...
                <div className="space-y-6 md:space-y-8 animate-in slide-in-from-right-4 pb-4">
                  <div className="space-y-3 md:space-y-4">
                    ...
                    <div className="p-3 md:p-4 rounded-xl border border-white/10 bg-black/30 space-y-3 md:space-y-4">
                      <label className="flex items-center gap-3 cursor-pointer"><input type="checkbox" checked={formData.requireVat} onChange={e => setFormData({...formData, requireVat: e.target.checked})} className="w-4 h-4 md:w-5 md:h-5 rounded border-gray-400 text-indigo-600 focus:ring-indigo-500 bg-white/10" /><span className="text-xs md:text-sm font-bold">Require Corporate Invoice (+{systemSettings.taxRate}% VAT)</span></label>
                      {formData.requireVat && (
                  ...
                  {deliveryType === "Delivery" && (
                    <div className="space-y-3 md:space-y-4 animate-in fade-in">
                      <div className="relative"><MapPin className="absolute left-3 top-1/2 -translate-y-1/2 opacity-50" size={16} /><select required value={formData.region} onChange={e => setFormData({...formData, region: e.target.value})} className="w-full pl-10 pr-4 py-3 bg-black border border-white/10 rounded-xl outline-none focus:border-indigo-500 text-base md:text-sm appearance-none"><option value="" disabled>Select Region</option>{ethiopianRegions.map(r => <option key={r} value={r}>{r}</option>)}</select></div>
                      {formData.region === "Addis Ababa" && <div className="relative"><LocateFixed className="absolute left-3 top-1/2 -translate-y-1/2 opacity-50" size={16} /><select required value={formData.subCity} onChange={e => setFormData({...formData, subCity: e.target.value})} className="w-full pl-10 pr-4 py-3 bg-black border border-white/10 rounded-xl outline-none focus:border-indigo-500 text-base md:text-sm appearance-none"><option value="" disabled>Select Sub-City</option>{addisSubcities.map(sc => <option key={sc} value={sc}>{sc}</option>)}</select></div>}
              ...
              <div className="p-4 md:p-6 bg-black/90 border-t border-white/10 shadow-[0_-10px_30px_rgba(0,0,0,0.8)] z-10">
                {formData.requireVat && <><div className="flex justify-between items-center text-xs md:text-sm opacity-70 mb-2 border-b border-white/5 pb-2"><span>Subtotal</span><span>{cartSubtotal.toLocaleString()} ETB</span></div><div className="flex justify-between items-center text-xs md:text-sm text-emerald-400 mb-3 md:mb-4"><span>VAT ({systemSettings.taxRate}%)</span><span>+ {vatAmount.toLocaleString()} ETB</span></div></>}
                <div className="flex justify-between items-end mb-4"><span className="opacity-50 font-medium uppercase tracking-widest text-[10px] md:text-xs">Total Pipeline</span><span className="text-xl md:text-2xl font-black">{cartTotal.toLocaleString()} ETB</span></div>
                <div className="flex gap-2 md:gap-3">
                  ...
                  <button type="submit" disabled={isCheckingOut} className="flex-1 py-3 md:py-4 rounded-xl text-black bg-white font-black uppercase tracking-widest text-xs md:text-sm transition-transform active:scale-95 flex items-center justify-center gap-2 md:gap-3 shadow-xl">{isCheckingOut ? <Loader2 className="animate-spin" size={16} /> : <CheckCircle2 size={16} />}{isCheckingOut ? "Connecting..." : "Pay via Chapa"}</button>
                </div>
              </div>
            </form>
          )}
        ...
        <div className="max-w-[1400px] mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
          <div className="md:col-span-2">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center font-black text-white overflow-hidden" style={{ backgroundColor: 'var(--accent)' }}>{systemSettings.logoUrl ? <img src={systemSettings.logoUrl} className="w-full h-full object-cover" alt="Logo" /> : "AZ"}</div>
