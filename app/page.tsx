...
export default function PremiumStorefront() {
  ...
  const catalogTree = useMemo(() => {
    ...
    Object.keys(PREDEFINED_MATRIX).forEach(menu => {
      tree[menu] = {};
      PREDEFINED_MATRIX[menu].forEach(submenu => { tree[menu][submenu] = new Set(); });
    });
    products.forEach(p => {
  ...
  const handleCheckout = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        items: cartItems, customerName: formData.name, companyName: formData.companyName,
        tinNumber: formData.tinNumber, requireVat: formData.requireVat, phone: formData.phone,
        totalAmount: cartSubtotal, deliveryType, region: formData.region, subCity: formData.subCity,
        specificAddress: formData.address, baseDeliveryFee: systemSettings.deliveryBaseFee
      };
          ...
          {!loading && filteredProducts.length > 0 && (
            <div className="flex justify-between items-center mb-4 md:mb-6">
              ...
              <div className="flex bg-[#111111] border border-white/10 rounded-lg p-1 shadow-lg">
                <button onClick={() => setViewMode('grid')} className={`p-1.5 md:p-2 rounded-md transition-all ${viewMode === 'grid' ? 'bg-white/10 text-white shadow' : 'text-gray-500 hover:text-gray-300'}`}>
                  <LayoutGrid size={16} />
                </button>
                <button onClick={() => setViewMode('list')} className={`p-1.5 md:p-2 rounded-md transition-all ${viewMode === 'list' ? 'bg-white/10 text-white shadow' : 'text-gray-500 hover:text-gray-300'}`}>
                  <ListIcon size={16} />
                </button>
              </div>
            </div>
                ...
                <div key={item.id} className={`group rounded-[1.5rem] md:rounded-[2rem] p-4 md:p-6 bg-[#111111] border border-white/10 transition-transform duration-300 hover:shadow-2xl relative overflow-hidden animate-in fade-in zoom-in transform-gpu ${viewMode === 'grid' ? 'flex flex-col hover:-translate-y-2' : 'flex flex-row gap-4 md:gap-6 items-center hover:-translate-y-1'}`} style={{ animationDelay: `${(i % 10) * 30}ms` }}>
                  ...
                  {item.imageUrl ? (
                    ...
                    <div className={`overflow-hidden bg-black/40 border border-white/5 flex flex-col items-center justify-center text-white/30 flex-shrink-0 ${viewMode === 'grid' ? 'w-full h-48 md:h-56 mb-4 md:mb-6 rounded-[1rem] md:rounded-2xl' : 'w-24 h-24 md:w-32 md:h-32 rounded-xl md:rounded-2xl'}`}>
                      ...
                      {viewMode === 'grid' && <span className="text-[10px] md:text-xs uppercase tracking-widest font-bold">No Showcase Asset</span>}
                    </div>
                  )}
                        ...
                        <div key={item.id} className="flex gap-3 md:gap-4 items-center bg-[#111111] border border-white/10 p-2.5 md:p-3 rounded-2xl">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {(item.size || item.color) && (
                              <p className="text-[8px] md:text-[9px] uppercase tracking-widest opacity-50 mb-1 truncate">
                                {item.size && <span className="mr-2">[{item.size}]</span>}{item.color && <span>[{item.color}]</span>}
                              </p>
                            )}
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              ...
                              <input type="number" min="1" value={item.quantity || 1} onChange={(e) => updateCartQuantity(item.id, parseInt(e.target.value) || 1)} className="w-14 md:w-16 px-1 md:px-2 py-0.5 bg-black border border-white/10 rounded text-[10px] md:text-xs font-bold text-center outline-none focus:border-[var(--accent)]" />
                            </div>
                          </div>
