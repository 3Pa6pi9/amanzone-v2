...
// =======================================================================
// NEW: ISOLATED PRODUCT CARD COMPONENT FOR PRE-CART CONFIGURATION
// =======================================================================
const StorefrontProductCard = ({ item, viewMode, onAddToCart }: { item: any, viewMode: string, onAddToCart: (item: any) => void }) => {
  // Parse comma-separated colors and sizes entered by admin
  ...
  const handlePushToCart = () => {
    onAddToCart({
      ...item,
      // Create a unique Cart ID so different sizes/colors don't overwrite each other
      cartItemId: `${item.id}-${selectedSize}-${selectedColor}`,
      quantity: localQty,
      selectedColor,
      selectedSize
    });
    setLocalQty(1); // Reset after adding
  };

  return (
    <div className={`group rounded-[1.5rem] md:rounded-[2rem] p-4 md:p-6 bg-[#111111] border border-white/10 transition-transform duration-300 hover:shadow-2xl relative overflow-hidden flex flex-col ${viewMode === 'grid' ? 'hover:-translate-y-2' : ''}`}>
      {item.imageUrl ? (
      ...
      <div className="flex flex-col flex-1">
        ...
        <div className={`mt-auto flex justify-between items-end pt-4 border-t border-white/10`}>
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center bg-black border border-white/10 rounded-lg p-0.5 h-full">
              <button onClick={() => setLocalQty(Math.max(1, localQty - 1))} className="px-2 py-1 text-gray-400 hover:text-white">-</button>
              <span className="text-[10px] md:text-xs font-bold w-6 text-center">{localQty}</span>
              <button onClick={() => setLocalQty(localQty + 1)} className="px-2 py-1 text-gray-400 hover:text-white">+</button>
            </div>
...
export default function PremiumStorefront() {
  ...
  // --- UPDATED CART LOGIC FOR UNIQUE CONFIGURATIONS ---
  const handleAddToCart = (configuredProduct: any) => {
    setCartItems(prev => {
      // Check if this exact configuration (size & color) already exists in cart
  ...
  const updateCartQuantity = (cartItemId: string, newQuantity: string | number) => {
    if (newQuantity === "") {
    ...
    if (!isNaN(parsed) && parsed >= 1) {
      setCartItems(prev => prev.map(item => item.cartItemId === cartItemId ? { ...item, quantity: parsed } : item));
    }
  };

  const handleQuantityBlur = (cartItemId: string, currentQuantity: any) => {
  ...
  const handleAiSubmit = async (e: React.FormEvent) => {
    ...
    } catch (error) { setAiHistory(prev => [...prev, {role: 'ai', text: "Neural link disrupted. Please try again or contact support."}]); } 
  ...
  return (
    <div className="relative min-h-screen font-sans scroll-smooth" style={{ backgroundColor: 'var(--bg-primary)', color: 'var(--text-main)' }}>
      ...
      {marketingAssets.length > 0 && (
        <section className="w-full max-w-[1400px] mx-auto px-4 md:px-6 mb-12">
          <div className="flex gap-4 overflow-x-auto snap-x scrollbar-hide pb-4">
            {marketingAssets.filter(ad => ad.active).map(ad => (
              <div key={ad.id} className="snap-center shrink-0 w-[85vw] md:w-[600px] lg:w-[800px] h-[200px] md:h-[300px] lg:h-[400px] rounded-[2rem] overflow-hidden border border-white/10 relative shadow-2xl">
                {ad.type === 'video' ? (
                  <video src={ad.url} autoPlay loop muted playsInline className="w-full h-full object-cover" />
                ) : (
        ...
        <div className="flex-1 min-h-screen">
          {!loading && filteredProducts.length > 0 && (
            <div className="flex justify-between items-center mb-4 md:mb-6">
              ...
              {/* Force Grid Mode Only Since List Mode breaks component structure slightly */}
            </div>
          )}
                        ...
                        <div key={item.cartItemId} className="flex gap-3 md:gap-4 items-center bg-[#111111] border border-white/10 p-2.5 md:p-3 rounded-2xl">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-1 mt-1 bg-black rounded-lg p-0.5 border border-white/10 w-fit">
                              <button type="button" onClick={() => updateCartQuantity(item.cartItemId, (parseInt(item.quantity) || 1) - 1)} disabled={(parseInt(item.quantity) || 1) <= 1} className="px-2 py-1 text-gray-400 hover:text-white hover:bg-white/10 rounded disabled:opacity-30">-</button>
                              <input type="number" min="1" value={item.quantity} onChange={(e) => updateCartQuantity(item.cartItemId, e.target.value)} onBlur={(e) => handleQuantityBlur(item.cartItemId, e.target.value)} className="w-10 bg-transparent text-[10px] md:text-xs font-bold text-center outline-none" style={{ MozAppearance: 'textfield' }} />
                              <button type="button" onClick={() => updateCartQuantity(item.cartItemId, (parseInt(item.quantity) || 0) + 1)} className="px-2 py-1 text-gray-400 hover:text-white hover:bg-white/10 rounded">+</button>
                            </div>
                          </div>
