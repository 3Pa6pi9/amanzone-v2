...
import { 
  ...
  TrendingUp, Truck, MapPin, Phone, User, FileText, ChevronRight, UploadCloud, Building2, ChevronDown, Menu, Mail, Lock, Briefcase, Clock, Printer, Megaphone, Video, Scissors
} from "lucide-react";
...
const initialFormState = {
  ...
  allowCustomSize: false, customSizeLabel: "Length (m)"
};
...
export default function AdminCommandCenter() {
  ...
  const openEditMenu = (product: any) => {
    setFormData({
      ...
      allowCustomSize: product.allowCustomSize || false, customSizeLabel: product.customSizeLabel || "Length (m)"
    });
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try { 
      ...
      setFormData(prev => ({ ...prev, imageUrl: data.url })); showToast("Asset uploaded."); 
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try { 
      const data = await uploadToCloudinary(file);
      setSystemSettings((prev: any) => ({ ...prev, logoUrl: data.url })); showToast("Logo uploaded."); 
  ...
  const handleMarketingUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      showToast(`Deploying to ${adPlacement}...`);
  ...
  const handleSaveInventory = async (e: React.FormEvent) => {
    ...
    try {
      const payload: any = { 
        ...
        allowCustomSize: formData.allowCustomSize, customSizeLabel: formData.customSizeLabel,
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await deleteDoc(doc(db, "inventory", id)); showToast("Material purged."); } 
    ...
    <div className="flex min-h-screen bg-[#050505] text-white font-sans selection:bg-emerald-500/30 overflow-x-hidden">
      ...
      <main className="flex-1 lg:ml-64 p-4 lg:p-8 pt-24 lg:pt-8 w-full">
                  ...
                  filteredProducts.slice(0, 50).map((product) => (
                    <div key={product.id} className="grid grid-cols-1 lg:grid-cols-12 gap-3 lg:gap-4 p-4 items-center hover:bg-white/5 transition-colors">
                      <div className="col-span-1 lg:col-span-4 flex items-start lg:items-center gap-3">
                        <div className="w-12 h-12 md:w-10 md:h-10 rounded-lg bg-black border border-white/10 overflow-hidden flex-shrink-0 flex items-center justify-center">{product.imageUrl ? <img src={optimizeImg(product.imageUrl)} className="w-full h-full object-cover" /> : <ImageIcon size={16} className="opacity-30" />}</div>
                        <div className="flex-1">
                          <p className="font-bold text-sm line-clamp-2 leading-tight">{product.title || "Unnamed"}</p>
                          {product.allowCustomSize && <span className="inline-block mt-1 text-[8px] bg-indigo-500/20 text-indigo-400 px-1.5 py-0.5 rounded border border-indigo-500/20">Custom Cut Enabled</span>}
                          <div className="flex flex-wrap items-center gap-1.5 mt-1 lg:hidden"><span className="text-[9px] font-bold uppercase text-emerald-400">{product.warehouse || "Central"}</span><span className="text-[9px] font-bold px-1.5 py-0.5 bg-white/10 rounded">QTY: {product.stock || 0}</span><span className="text-[9px] font-bold px-1.5 py-0.5 bg-white/10 rounded">{product.price || 0} ETB</span></div>
                        </div>
                      </div>
        ...
        {activeTab === "marketing" && (
          <div className="animate-in fade-in duration-300">
            <header className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mb-6 md:mb-8">
              <div><h2 className="text-2xl md:text-3xl font-black tracking-tight mb-1">Marketing Engine</h2><p className="text-xs md:text-sm opacity-50 font-medium">Control the Storefront display matrix.</p></div>
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <div className="relative">
                  ...
                  <button onClick={() => document.getElementById('marketingUpload')?.click()} disabled={isUploadingMarketing} className="w-full sm:w-auto px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-black text-sm uppercase tracking-widest rounded-xl transition-all active:scale-95 flex items-center justify-center gap-2 disabled:opacity-50 shadow-[0_0_20px_rgba(79,70,229,0.3)]">
                    {isUploadingMarketing ? <Loader2 className="animate-spin" size={16} /> : <UploadCloud size={16} />} Deploy to {adPlacement}
                  </button>
                </div>
              </div>
            </header>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {marketingAssets.length === 0 ? (
                ...
                marketingAssets.map(asset => (
                  <div key={asset.id} className="bg-[#0A0A0F] border border-white/10 rounded-2xl overflow-hidden shadow-xl group flex flex-col">
                    <div className="aspect-video bg-black relative flex-shrink-0">
                      {asset.type === 'video' ? (
                        ...
                        <img src={optimizeImg(asset.url)} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" />
                      )}
                    ...
                    <div className="p-4 flex justify-between items-center bg-[#111111] border-t border-white/10 flex-1">
                      <span className="text-[10px] font-bold uppercase tracking-widest text-indigo-400 bg-indigo-500/10 px-2 py-1 rounded border border-indigo-500/20">Target: {asset.placement || 'inline'}</span>
      ...
      {/* ========================================================= */}
      {/* DRAWER: INVENTORY FORM WITH CUSTOM LENGTH SETTINGS */}
      {/* ========================================================= */}
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-3 md:gap-4 items-center">
                    <div onClick={() => document.getElementById('imageUpload')?.click()} className="relative w-14 h-14 md:w-16 md:h-16 rounded-xl bg-black border border-white/10 overflow-hidden flex-shrink-0 flex items-center justify-center group cursor-pointer hover:border-emerald-500 transition-colors">
                      {formData.imageUrl ? <img src={optimizeImg(formData.imageUrl)} className="w-full h-full object-cover" /> : <UploadCloud size={20} className="opacity-30 group-hover:text-emerald-400 group-hover:opacity-100" />}
                ...
                <div className="p-4 rounded-xl border border-yellow-500/20 bg-yellow-900/10 space-y-4">
                  ...
                  <div className="pt-2 space-y-4">
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input type="checkbox" checked={formData.allowCustomSize} onChange={e => setFormData({...formData, allowCustomSize: e.target.checked})} className="w-4 h-4 rounded border-gray-400 text-yellow-500 focus:ring-yellow-500 bg-white/10" />
                      <span className="text-sm font-bold text-yellow-100">Enable Custom Length Input for Client</span>
                    </label>
                    {formData.allowCustomSize && (
                      <div className="space-y-2 animate-in fade-in">
                        <label className="text-[10px] font-bold uppercase tracking-widest opacity-70">Input Label</label>
                        <input type="text" placeholder="e.g. Length (Meters)" value={formData.customSizeLabel} onChange={e => setFormData({...formData, customSizeLabel: e.target.value})} className="w-full px-3 py-2 bg-black border border-white/10 rounded-lg outline-none focus:border-yellow-500 text-base md:text-sm" />
                        <p className="text-[9px] opacity-50 text-yellow-200">The client's input will be multiplied by the base price.</p>
                      </div>
                    )}
                  </div>
                </div>
                          ...
                          <div className="flex justify-between items-start w-full">
                            <div className="flex-1 pr-2">
                              ...
                              <p className="text-[10px] md:text-xs opacity-50 mt-0.5">
                                {(Number(item.quantity) || 1)} {item.metric || 'Units'} 
                                {item.customLength && ` x ${item.customLength} ${item.customSizeLabel}`} 
                                @ {(Number(item.price)||0).toLocaleString()} ETB
                              </p>
                            </div>
                            <p className="font-black text-emerald-400 text-sm whitespace-nowrap">
                              {((Number(item.price)||0) * (Number(item.quantity) || 1) * (Number(item.customLength) || 1)).toLocaleString()}
                            </p>
                          </div>
              ...
              <table className="w-full text-left mb-12">
                <thead>
                  <tr className="border-b-2 border-gray-800 text-gray-800">
                    ...
                    <th className="py-3 px-2 font-bold uppercase tracking-widest text-xs text-right w-32">Qty & Specs</th>
                  ...
                  {selectedOrder.items?.map((item:any, i:number) => (
                    <tr key={i} className="text-gray-800">
                      ...
                      <td className="py-4 px-2 text-right">
                        {(Number(item.quantity) || 1)} {item.metric || 'Units'}
                        {item.customLength && <span className="block text-[10px] text-gray-500 mt-0.5">{item.customLength} {item.customSizeLabel}</span>}
                      </td>
                      <td className="py-4 px-2 text-right">{(Number(item.price)||0).toLocaleString()}</td>
                      <td className="py-4 px-2 text-right font-bold">{((Number(item.price)||0) * (Number(item.quantity) || 1) * (Number(item.customLength) || 1)).toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
